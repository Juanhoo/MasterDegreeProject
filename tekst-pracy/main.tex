...
% TODO
\newcommand{\FirstName}{Piotr}
\newcommand{\Surname}{Kałucki}
\newcommand{\Supervisor}{$\langle$dr hab. inż, prof PŚ Krzysztof Simiński$\rangle$}     % dane promotora (bez $\langle$ i $\rangle$)
\newcommand{\Title}{TGranularny hierarchiczny algorytm grupowania aglomeracyjnego}           % tytuł pracy po polsku
\newcommand{\TitleAlt}{Granular hierarchical agglomerative clustering algorithm}                     % thesis title in English
\newcommand{\Program}{$\langle$Informatyka$\rangle$}            % kierunek studiów  (bez $\langle$ i $\rangle$)
\newcommand{\Specialisation}{$\langle$Oprogramowanie Systemowe$\rangle$}     % specjalność  (bez $\langle$ i $\rangle$)
\newcommand{\Id}{$\langle$262880$\rangle$}                 % numer albumu  (bez $\langle$ i $\rangle$)
\newcommand{\Departament}{$\langle$Katedra Algorytmiki i Oprogramowania$\rangle$}        % katedra promotora  (bez $\langle$ i $\rangle$)
